    ...
    void generate_random_string(uint8_t *str, int length) {
        uint32_t index;
        for (int i = 0; i < length; ++i) {
            index = esp_random() % (sizeof(charset) - 1); // this is a TRNG
            str[i] = charset[index];
        }
        str[length] = '\0';
    }
    ...
    void signature_generater(uint8_t *data_to_sign,uint8_t *signature)
    {
        // key_pair_checker();
        // uint8_t data_to_sign[16] = "A"; // this will be a random string but now it's just testing.
        // uint8_t data_to_sign[] = "\x41"; 
        generate_random_string(data_to_sign,16);
        ...
        /* 
        *if you need to verify your signature:
